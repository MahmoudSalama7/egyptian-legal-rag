...
def clean_text(text: str) -> str:
    ...
    filtered_lines = [l for l in lines if l and l != "ا"]
    return "\n".join(filtered_lines)
...
def clean_arabic_article_text(raw_ar: str, art_num: int) -> str:
    ...
    for l in lines:
        ...
        if line_str in {"القانون المدني المصري", "نصوص القانون المدنى", "قانون الإصدار"}:
            continue

        norm = normalize_digits(line_str)
        # إزالة أشكال رؤوس المواد المقلوبة أو ذات الأقواس في البداية: مثل "٠١(" أو "مادة ( )١"
        if not skipped_header:
            header_pattern = re.search(r"^(?:\([ \t]*)?(?:مادة|ماده)?[\(\)\s]*([0-9]{1,4})[\(\)\s]*(.*)$", norm)
            if header_pattern:
                found_num = int(header_pattern.group(1)) if header_pattern.group(1) else None
                # إذا كان الرقم في بداية السطر يطابق رقم المادة الحالية
                if found_num == art_num or (found_num and str(found_num) in {str(art_num), str(art_num)[::-1]}):
...
def get_precise_english_markers(page: pymupdf.Page, mid_x: float, min_y: float, max_y: float) -> list[tuple[float, int]]:
    """
    ...
    for (b_no, l_no), line_words in lines_dict.items():
...
def extract_articles(pdf_path: str | Path) -> list[dict]:
    ...
    for page_idx in range(len(doc)):
        ...
        if page_num == 1 and "قانون الإصدار" in raw_text and "نصوص القانون المدنى" not in raw_text:
            continue
        ...
        for i, (y_pos, art_num) in enumerate(article_markers):
            ...
            next_y = article_markers[i + 1][0] if i + 1 < len(article_markers) else max_y
            ...
            clean_en_lines = []
            for l in raw_en_clip.splitlines():
                if match_english_article_header(l.strip()) == art_num:
                    continue
                if l.strip():
                    clean_en_lines.append(l.strip())
            en_body = clean_text("\n".join(clean_en_lines))
...
def main():
    parser = argparse.ArgumentParser(description="Extract Civil Code articles from PDF.")
    parser.add_argument("--input", required=True, help="Path to input Law.pdf")
    parser.add_argument("--output", required=True, help="Path to output civil_code.json")
